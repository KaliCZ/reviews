...
test('submit a 4-star review (auto-approved) and see it on the product page', async ({ page }) => {
  ...
  if ((await writeCta.count()) === 0) {
    await expect(page.getByRole('link', { name: /Edit your review/i })).toBeVisible();
  ...
  await expect.poll(
    async () => {
      await page.reload();
      return await page.locator('article.review .body', { hasText: body }).count();
    },
...
test('upvote a review and see the score change', async ({ page }) => {
  ...
  // can flip cleanly).
  const otherReview = page.locator('article.review').filter({
  ...
  const voteResp = page.waitForResponse(
    (r) => /\/api\/reviews\/[^/]+\/vote$/.test(r.url()) && r.request().method() === 'POST',
  );
  ...
  // value moved.
  await expect.poll(
    async () => parseInt((await scoreLocator.textContent()) ?? '', 10),
