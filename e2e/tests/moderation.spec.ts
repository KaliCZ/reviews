...
test('5-star review waits for moderation, appears after Approve signal', async ({ page }) => {
  await page.goto(`/products/${productSlug}`);

  const writeCta = page.getByRole('link', { name: /Write a review/i });
  test.skip((await writeCta.count()) === 0,
  ...
  await page.goto(`/products/${productSlug}`);
  await expect(page.locator('article.review .body', { hasText: body })).toHaveCount(0);
